...
$zeugnisgewichtung=isset($_POST['zeugnisgewichtung']) && !empty($_POST['zeugnisgewichtung']) ? (int)$_POST['zeugnisgewichtung'] : -1;
...
if(!empty($fehler)) {
  $fehler['kurs']=(object)array(
    ...
    'zeugnisgewichtung'=>$zeugnisgewichtung,
...
if($id>0) {
  ...
  $stmt=$db->prepare("update gpb_kurs set titel=?,beginn=?,ende=?,einzeltage=?,raumid=?,moodleid=?,sichtbar=?,modulid=?,zeugnisrelevant=?,zeugnisgewichtung=?,notenstatus=?,planungnotiz=?,planungfarbe=? where id=? limit 1");
  $stmt->bind_param('sssiiiiiiisssi',$titel,$beginn,$ende,$einzeltage,$raumid,$moodleid,$sichtbar,$modulid,$zeugnisrelevant,$zeugnisgewichtung,$notenstatus,$planungnotiz,$planungfarbe,$id);
  $stmt->execute();
  if($moodleid>0 && !$moodleerstellen) {
    $daten=(object)array(
      'courseid'=>$moodleid,
  ...
  $stmt=$db->prepare("insert into gpb_kurs(titel,beginn,ende,einzeltage,raumid,moodleid,sichtbar,modulid,zeugnisrelevant,zeugnisgewichtung,notenstatus,planungnotiz,planungfarbe) values(?,?,?,?,?,?,?,?,?,?,?,?,?)");
  $stmt->bind_param('sssiiiiiiisss',$titel,$beginn,$ende,$einzeltage,$raumid,$moodleid,$sichtbar,$modulid,$zeugnisrelevant,$zeugnisgewichtung,$notenstatus,$planungnotiz,$planungfarbe);
  $stmt->execute();
  $id=$db->insert_id;
  if($istPV) {
    $db->query("insert ignore into gpb_pruefungsvorbereitung(kursid) values(".$id.")");
...
header('Location:'.(empty($redirect) ? 'kurs_sehen.php?kursid='.$id : $redirect));
exit;
?>