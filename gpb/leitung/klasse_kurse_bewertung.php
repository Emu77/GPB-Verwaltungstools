...
$stmt=$db->prepare("select * from gpb_kurs_view k where k.id in(select kursid from gpb_kurs_klasse where klasseid=".$klasse->id.") order by k.beginn,k.ende,k.titel");
...
if(!empty($kurse->byId)) {
  $result=$db->query("select * from gpb_bewertung where not stumm and kursid in(".implode(',',array_keys($kurse->byId)).")
    and tnid in(select ktn.tnid from gpb_klasse_tn ktn where ktn.klasseid=".$klasse->id.")");
...
$seite=new LeitungSeite('Klasse '.$klasse->bezeichnung.' - Modulbewertung');
...
?>
    <td><?= $anzahlen['summe']>0 ? sprintf('%0.1f',$werte['summe']/$anzahlen['summe']) : '' ?></td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  </tr>
  <tr class="summen">
...
?>
  <tr>
    <td valign="top">
      <?= $kurs->titel ?>
      <div style="text-align:right;"><?= $klasse->anzahlTN ?>&nbsp;TN</div>
    </td>
<?php
    foreach(Bewertung::$fragen as $frage=>$fragentext) {
?>
    <td align="center" valign="top"><?= isset($kurs->anzahlen[$frage]) ? sprintf('%0.1f',$kurs->werte[$frage]/$kurs->anzahlen[$frage]).'<br />'.$kurs->anzahlen[$frage].'&nbsp;('.round(100*$kurs->anzahlen[$frage]/$klasse->anzahlTN).'%)' : '' ?></td>
<?php
    }
?>
    <td align="center"><b><?= isset($kurs->anzahlen) && $kurs->anzahlen['summe']>0 ? sprintf('%0.1f',$kurs->werte['summe']/$kurs->anzahlen['summe']).'<br />'.count($kurs->bewerterids).'&nbsp;('.round(100*count($kurs->bewerterids)/$klasse->anzahlTN).'%)' : '' ?></b></td>
