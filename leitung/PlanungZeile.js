  ...
  addKurs(kurs) {
    ...
    for(let kw of kurs.kw) {
      if(typeof this.tdsByKw[kw]!='undefined') {
        ...
        geplant.setKompakt(this.kompakt);
        this.tdsByKw[kw].childNodes[1].style.display='none';
      }
    }
  }
  removeKurs(kurs) {
    ...
    for(let kw of kurs.kw) {
      let td=this.tdsByKw[kw];
      if(typeof td!='undefined') {
        td.childNodes[1].style.display=this.kompakt && td.childNodes[0].childNodes.length>0 ? 'none' : '';
      }
    }
  }
  
  setKompakt(kompakt) {
    if(this.kompakt==kompakt) return;
    this.kompakt=kompakt;
    if(kompakt) {
      this.tr.classList.add('kompakt');
    } else {
      this.tr.classList.remove('kompakt');
    }
    this.th.childNodes[0].innerText=this.wer.makeThText();
    for(let kursid in this.geplanteByKursid) {
      for(let geplantes of this.geplanteByKursid[kursid]) {
        geplantes.setKompakt(kompakt);
      }
    }
    for(let kw in this.tdsByKw) {
      let td=this.tdsByKw[kw];
      td.childNodes[1].style.display=this.kompakt && td.childNodes[0].childNodes.length>0 ? 'none' : '';
    }
  }
