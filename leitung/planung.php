...
<div id="konfigauswahl">
  ...
  function konfig_umbenennen() {
    let bez=prompt('Neue Bezeichnung:','<?= $konfig ? str_replace("'","\\'",$konfig->bezeichnung) : '' ?>');
    if(!bez) return;
    location.href='planungkonfig_aendern.php?bezeichnung='+encodeURIComponent(bez);
  }
  function konfig_erstellen(kopie) {
    let bez=prompt('Bezeichnung der neuen Konfig:',kopie ? '<?= $konfig ? str_replace("'","\\'",$konfig->bezeichnung) : '' ?>' : '');
    if(!bez) return;
