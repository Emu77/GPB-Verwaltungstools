...
// Gruppensummen für die ausgewählten Kurse berechnen
$werteDozent=$anzahlenDozent=0;
foreach($fragenDozent as $frage) {
  if(isset($anzahlen[$frage])) {
    $werteDozent += $werte[$frage];
    $anzahlenDozent += $anzahlen[$frage];
  }
}
$werteSchule=$anzahlenSchule=0;
foreach($fragenSchule as $frage) {
  if(isset($anzahlen[$frage])) {
    $werteSchule += $werte[$frage];
    $anzahlenSchule += $anzahlen[$frage];
  }
}
  ...
  foreach($kurse->alle as $kurs) {
    ...
    foreach($fragenDozent as $frage) {
      if(isset($kurs->anzahlen[$frage])) {
        $kursWerteDozent += $kurs->werte[$frage];
        $kursAnzahlenDozent += $kurs->anzahlen[$frage];
      }
    }
    $kursWerteSchule=$kursAnzahlenSchule=0;
    foreach($fragenSchule as $frage) {
      if(isset($kurs->anzahlen[$frage])) {
        $kursWerteSchule += $kurs->werte[$frage];
