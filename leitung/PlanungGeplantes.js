...
export class PlanungGeplantes {
  ...
  setKompakt(kompakt){
    ...
    if(this.kurs.modulid>0) {
      let m=PlanungModul.byId[this.kurs.modulid];
      m.textKorrigieren(this.modulSpan.childNodes[0]);
    }
    let idx=0;
    for(let obj of this.kurs.dozenten) {
      let d=PlanungObject.ladeEines(PlanungDozent,obj);
      d.textKorrigieren(this.dozentenSpan.childNodes[idx]);
      ++idx;
    }
    if(kompakt) {
      this.klassenDiv.style.display='none';
      this.raumDiv.style.display='none';
      this.notizDiv.style.display='none';
      if(this.kurs.planungfarbe) {
        this.div.style.backgroundColor=this.kurs.planungfarbe;
      }
    } else {
      this.klassenDiv.style.display='';
      this.raumDiv.style.display='';
      this.notizDiv.style.display='';
