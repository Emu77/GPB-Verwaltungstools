...
export class PlanungFerien {
  ...
  makeDisplay(parentNode) {
    ...
    div.ferien=this;
    div.innerText=this.makeText();
    if(parentNode) parentNode.appendChild(div);
  }
  displayKorrigieren(div) {
    div.innerText=this.makeText();
  }
  makeText() {
    if(PlanungFerien.planung.kompakt) return this.anlass;
    return PlanungFerien.formatDate(this.beginn)+(this.ende==this.beginn ? '' : '-'+PlanungFerien.formatDate(this.ende))+' '+this.anlass;
  }
  install(planung) {
  ...
  kompaktGeaendert() {
    if(this.art=='Klassenferien') {
      for(let klasse of this.klassen) {
        ...
        if(!zeile) continue;
        for(let kw of this.kw) {
          let woche=PlanungFerien.planung.wochenByKw[kw];
          if(!woche) continue;
//          if(this.beginn<=woche.montag && this.ende>=woche.freitag) {
//            zeile.tr.cells[woche.tdidx].classList.add('ferien');
//          }
          let td=zeile.tr.cells[woche.tdidx];
          for(let i=td.childNodes.length-1;i>=2;--i) {
            if(td.childNodes[i].ferien==this) {
              td.childNodes[i].innerText=this.makeText();
            }
          }
        }
      }
